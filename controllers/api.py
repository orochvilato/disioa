...
def _ajax(type_page):
    ...
    cachekey= u"%s_%s_%s_%s_%s_%s" % (type_page,age,csp.decode('utf8') if csp else csp,groupe,text,region.decode('utf8') if region else region)
    counts = cache.ram(cachekey,lambda:countItems(),time_expire=0)
