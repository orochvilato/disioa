...
def getvisuel(name,deputeid,**args):
    ...
    groupe_abrev = depute['groupe_abrev']
    if groupe_abrev == 'NI':
        groupe_abrev = depute['depute_election']['nuance']
        groupe_lib = nuances[groupe_abrev]
    else:
        groupe_lib = depute['groupe_libelle'].split(':')[0].strip()
    #groupe_abrev = "UDI"
    #groupe_lib = nuances[groupe_abrev]
    ...
    pct = ("%d%%" % depute['stats']['positions']['exprimes']).replace('.',',')
    ...
    svg = svgvisuel(name,symbol=symbol,photo=photo,prenom=prenom,nom=nom,pct=pct,groupe=groupe,date=datetime.datetime.now().strftime('%d/%m/%Y'))
    data = str(svg)
