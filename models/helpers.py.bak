...
def svggauge(typeg,pct,unit='%'):
    if typeg=='compfi':
        ...
        symbol = typeg
        transp ='0.33'
    return XML(response.render('svg/gauge.svg',color=circlecolor,symbol=XML(response.render('svg/gauge/%s.svg' % symbol, color=symbolcolor)),pct=int(round(pct,0)) if pct!=None else None,transp=transp,unit=unit))
